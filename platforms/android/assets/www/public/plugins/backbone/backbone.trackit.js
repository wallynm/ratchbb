(function() {
  ...
  Backbone.History.prototype.navigate = _.wrap(Backbone.History.prototype.navigate, function(oldNav, fragment, options) {
    ...
    if (prompt) {
      ...
      /*BootstrapDialog.confirm(undefined, '<div style="font-size:31px;text-align: center;">'+prompt+'</div><div style="font-size:15px; text-align:center; width: 400px; margin: 10px auto 0px auto;">Frase</div>', {cancel: 'Cancelar', ok: 'OK'}, function(result){
        if(result) {
          unsavedModels[0].stopTracking();
